...
ui::ListView* UI::listView(const std::vector<std::string>& items, int x, int y, int w, int h,
                           int itemHeight) {
...
ui::Image* UI::image(const unsigned char* data, size_t dataSize, int x, int y, int w, int h,
                     bool stretch) {
...
ui::Sprite* UI::sprite(const unsigned char* data, size_t dataSize, int x, int y, int w, int h,
                       bool stretch) {
...
ui::AnimatedSprite* UI::animatedSprite(const std::string& path, int x, int y, int w, int h,
    ...
    auto animatedSpritePtr =
        createAnimatedSprite(path, x, y, w, h, frameCount, frameDelay, stretch);
    return animatedSpritePtr.get();
}

ui::AnimatedSprite* UI::animatedSprite(const unsigned char* data, size_t dataSize, int x, int y,
                                       int w, int h, int frameCount, Uint32 frameDelay,
                                       bool stretch) {
    auto animatedSpritePtr =
        createAnimatedSprite(data, dataSize, x, y, w, h, frameCount, frameDelay, stretch);
    return animatedSpritePtr.get();
}

ui::ProgressBar* UI::progressBar(int x, int y, int width, int height, float initValue,
                                 bool showText) {
    auto progressBarPtr = createProgressBar(x, y, width, height, initValue, showText);
    return progressBarPtr.get();
}

ui::Modal* UI::modal(const std::string& message, const std::string& buttonText, bool hasCancel,
                     std::function<void()> onCloseCallback) {
    auto modalPtr = createModal(message, buttonText, hasCancel, onCloseCallback);
    return modalPtr.get();
}

ui::Modal* UI::confirmModal(const std::string& message, std::function<void()> onConfirmCallback,
                            std::function<void()> onCancelCallback) {
