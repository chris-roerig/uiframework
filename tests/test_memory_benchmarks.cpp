...
TEST_CASE("Memory Usage Benchmarks", "[benchmark][memory]") {
    ...
    SECTION("Memory efficiency metrics") {
        ...
        BENCHMARK("Memory per widget (average)") {
            ...
            // Create 1000 mixed widgets
            for (int i = 0; i < 250; ++i) {
                int x = (i % 25) * 30;
                int y = (i / 25) * 30;
                
                widgets.push_back(ui.createLabel("L" + std::to_string(i), x, y));
                widgets.push_back(ui.createButton("B" + std::to_string(i), x + 200, y, [](){}));
                widgets.push_back(ui.createTextBox("T" + std::to_string(i), x + 400, y));
                widgets.push_back(ui.createCheckBox(false, x + 600, y, [](bool){}));
            }
    ...
    SECTION("Widget memory scaling") {
        ...
        for (int count = 0; count <= 500; count += 50) {
            ...
            for (int i = 0; i < count; ++i) {
                labels.push_back(ui.createLabel("Label", i % 40 * 20, i / 40 * 25));
            }
